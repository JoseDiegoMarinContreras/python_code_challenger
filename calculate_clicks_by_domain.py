...
So "mail" and "mail.yahoo" are not valid domains. Note that "mobile.sports" appears as a separate domain near the bottom
of the input.)
...
def calculate_clicks_by_domain(domains: list) -> list:
    domains_clicks = {}
    for domain_stats in domains:
        stats = domain_stats.split(",")
        clicks = int(stats[0])
        subdomains = stats[1].split(".")
        subdomains.reverse()
        _subdomain = ""
        for subdomain in subdomains:
            _subdomain = subdomain if _subdomain == "" else subdomain + "." + _subdomain
            domains_clicks[_subdomain] = domains_clicks[_subdomain] + clicks if _subdomain in domains_clicks else clicks

    return domains_clicks
